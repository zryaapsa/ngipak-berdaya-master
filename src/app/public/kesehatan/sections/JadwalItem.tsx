import type { JadwalKesehatan } from "../../../../features/kesehatan/types";
import { formatTanggalID } from "../../../../lib/date";
import Badge from "../../../../components/ui/Badge";

export default function JadwalItem({ j }: { j: JadwalKesehatan }) {
  const jam = j.jam_mulai ? `${j.jam_mulai}${j.jam_selesai ? `–${j.jam_selesai}` : ""}` : null;
    ...
    <div className="rounded-2xl border border-gray-100 bg-white p-4 transition hover:shadow-soft">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {j.lokasi ? <div className="mt-1 text-sm text-gray-600">{j.lokasi}</div> : null}
        </div>
